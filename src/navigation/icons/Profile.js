import * as React from 'react';
import Svg, { G, Path } from 'react-native-svg';
import { wp } from '../../constants/theme';

const SIZE = wp(6.8);

const ProfileIcon = (props) => {
  const { size } = props;
  return (
    <Svg width={size || SIZE} height={size || SIZE} viewBox="0 0 20 20" {...props}>
      <G fill="none" fillRule="evenodd">
        <Path d="M10 6c-1.178 0-2 .822-2 2s.822 2 2 2 2-.822 2-2-.822-2-2-2z" />
        <Path d="M10 12c-2.28 0-4-1.72-4-4 0-2.28 1.72-4 4-4 2.28 0 4 1.72 4 4 0 2.28-1.72 4-4 4z" />
        <Path
          d="M10 0C4.579 0 0 4.579 0 10c0 3.189 1.592 6.078 4 7.924l.102.076C5.77 19.245 7.813 20 10 20s4.23-.755 5.898-2l.102-.076c2.408-1.846 4-4.734 4-7.924 0-5.421-4.579-10-10-10zM5.528 17.097C5.825 15.752 7.568 14 9 14h2c1.432 0 3.368 1.753 3.665 3.097C13.497 17.774 11.67 18.5 10 18.5s-3.304-.726-4.472-1.403zM15.89 16.05c-.757-1.8-2.818-3.477-4.89-3.477H9.042c-2.072 0-4.043 1.678-4.799 3.477C2.776 14.591 1.5 13.176 1.5 10c0-4.59 3.91-8.5 8.5-8.5s8.5 3.91 8.5 8.5c0 3.176-1.143 4.591-2.61 6.05z"
          fill="#272D2F"
          fillRule="nonzero"
        />
        <Path
          d="M10 4.1C7.82 4.1 6.1 5.82 6.1 8c0 2.18 1.72 3.9 3.9 3.9 2.18 0 3.9-1.72 3.9-3.9 0-2.18-1.72-3.9-3.9-3.9zm0 6.4c-1.43 0-2.5-1.07-2.5-2.5S8.57 5.5 10 5.5s2.5 1.07 2.5 2.5-1.07 2.5-2.5 2.5z"
          fill="#272D2F"
          fillRule="nonzero"
        />
      </G>
    </Svg>
  );
};

const ProfileFilledIcon = (props) => {
  const { size } = props;
  return (
    <Svg width={size || SIZE} height={size || SIZE} viewBox="0 0 38 38" {...props}>
      <G fill="none" fillRule="evenodd">
        <Path d="M19 11.4c-2.238 0-3.8 1.562-3.8 3.8 0 2.238 1.562 3.8 3.8 3.8 2.238 0 3.8-1.562 3.8-3.8 0-2.238-1.562-3.8-3.8-3.8Z" />
        <Path d="M19 22.8c-4.332 0-7.6-3.268-7.6-7.6s3.268-7.6 7.6-7.6 7.6 3.268 7.6 7.6-3.268 7.6-7.6 7.6Z" />
        <Path
          d="M19 0C8.7 0 0 8.7 0 19c0 6.06 3.025 11.548 7.6 15.056l.194.144C10.963 36.565 14.844 38 19 38c4.155 0 8.037-1.434 11.206-3.8l.194-.144C34.975 30.548 38 25.06 38 19 38 8.7 29.3 0 19 0Zm11.19 30.495c-1.437-3.42-5.353-6.606-9.29-6.606h-3.72c-3.937 0-7.681 3.188-9.118 6.606C5.275 27.723 2.85 25.035 2.85 19c0-8.721 7.429-16.15 16.15-16.15 8.721 0 16.15 7.429 16.15 16.15 0 6.034-2.172 8.723-4.96 11.495Z"
          fill="#272D2F"
          fillRule="nonzero"
        />
        <Path
          d="M19 7.79c-4.142 0-7.41 3.268-7.41 7.41s3.268 7.41 7.41 7.41 7.41-3.268 7.41-7.41S23.142 7.79 19 7.79Z"
          fill="#272D2F"
          fillRule="nonzero"
        />
      </G>
    </Svg>
  );
};

export { ProfileIcon, ProfileFilledIcon };
