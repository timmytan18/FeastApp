...
import { StyleSheet, Text, View, SafeAreaView, TouchableOpacity, FlatList, Image, Keyboard } from 'react-native';
...
const NewPost = ({ navigation }) => {
    ...
    async function getNearby() {
        ...
        let url = `https://api.foursquare.com/v2/venues/search?ll=${lat},${lng}&limit=50&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${FS_VERSION}`;
        // let url = `https://api.foursquare.com/v2/venues/explore?ll=${lat},${lng}&limit=50&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${FS_VERSION}`;
    ...
    const searchRestaurant = async (query) => {
        ...
        if (query == '') {
            ...
            let url = `https://api.foursquare.com/v2/venues/search?ll=${lat},${lng}&query=${query}&radius=${radius}&limit=50&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&v=${FS_VERSION}`;
            let res = await fetch(url);
            let data = await res.json();
    ...
    async function filterSetResults(results) {
        ...
        let items = await filterFSItems(results);
    ...
    const renderItem = ({ item }) => {
        ...
        if (item.id == selected) {
            return (
                <TouchableOpacity style={styles.itemContainer} activeOpacity={0.95} onPress={() => {
                    setSelected(null)
                    ...
                    >
                        ...
                        <View style={styles.infoContainer}>
                            ...
                            <Text style={[styles.locationText, { color: 'white' }]} numberOfLines={item.name.length > 20 ? 1 : 2} ellipsizeMode='tail'>{loc}</Text>
                        </View>
                        <View style={styles.distanceContainer}>
                            <Car size={wp(4.5)} color='white' />
                            <Text style={[styles.distanceText, { color: 'white' }]}>{distance} mi</Text>
                        </View>
                    </LinearGradient>
                </TouchableOpacity>
            );
            ...
            return (
                <TouchableOpacity style={styles.itemContainer} activeOpacity={0.8} onPress={() => {
                    setSelected(item.id)
                    ...
                    <View style={styles.infoContainer}>
                        ...
                        <Text style={styles.locationText} numberOfLines={item.name.length > 20 ? 1 : 2} ellipsizeMode='tail'>{loc}</Text>
                    </View>
                    <View style={styles.distanceContainer}>
                        <Car size={wp(4.5)} />
            ...
            <View style={styles.container}>
                ...
                <View style={[styles.optionsContainer, { flexDirection: loading ? 'row' : 'column' }]}>
                    <LinearGradient
                        ...
                        style={styles.fade}
                    />
                    {loading && <BallIndicator style={{ alignSelf: 'flex-start', marginTop: wp(8) }} color={colors.tertiary} />}
                    ...
                    {!loading && restaurantList.length === 0 &&
                        <View style={styles.noResultsContainer}>
                            <Text style={styles.noResultsText}>Sorry, we couldn't find the restaurant you're looking for.</Text>
                            <Text style={styles.noResultsText}>Try searching for the exact restaurant name!</Text>
                        </View>
                    }
                </View>
            </View>
